      ...
      html: `
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#0a0a0a;color:#fff;border-radius:12px;overflow:hidden;">
          <div style="background:linear-gradient(135deg,#c9a84c,#f5d376);padding:24px;text-align:center;">
            ...
            <p style="margin:6px 0 0;color:#000;opacity:0.6;font-size:13px;">Prime Detailing — Ref #${contact.referenceId}</p>
          </div>
          <div style="padding:28px;">
            <table style="width:100%;border-collapse:collapse;margin-bottom:20px;">
              ...
              <tr><td style="padding:8px 0;color:rgba(255,255,255,0.5);font-size:13px;">Email</td><td style="padding:8px 0;"><a href="mailto:${contact.email}" style="color:#c9a84c;">${contact.email}</a></td></tr>
              ${contact.phone ? `<tr><td style="padding:8px 0;color:rgba(255,255,255,0.5);font-size:13px;">Phone</td><td style="padding:8px 0;"><a href="tel:${contact.phone}" style="color:#c9a84c;">${contact.phone}</a></td></tr>` : ''}
            ...
            <div style="text-align:center;">
              <a href="mailto:${contact.email}?subject=Re: ${contact.subject || 'Your inquiry'}" style="display:inline-block;background:linear-gradient(135deg,#c9a84c,#f5d376);color:#000;text-decoration:none;padding:12px 28px;border-radius:8px;font-weight:700;font-size:14px;">Reply to ${contact.name}</a>
            </div>
          </div>
          <div style="padding:16px;text-align:center;border-top:1px solid rgba(255,255,255,0.07);">
            <p style="margin:0;color:rgba(255,255,255,0.3);font-size:11px;">Manage in <a href="${process.env.FRONTEND_URL || 'http://localhost:5173'}/admin" style="color:#c9a84c;">Admin Panel</a> → Messages</p>
          </div>
        </div>`,
    });
  } catch (err) {
