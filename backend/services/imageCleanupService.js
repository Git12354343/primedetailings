...
async function scheduleImageDeletion(bookingId) {
  ...
  if (updated.count > 0) {
    console.log(`[ImageCleanup] Scheduled ${updated.count} images for deletion (bookingId=${bookingId})`);
  }
}
...
async function cleanupExpiredImages() {
  ...
  for (const img of expired) {
    try {
      ...
      if (error) {
        console.error(`[ImageCleanup] Storage delete failed for ${img.storagePath}:`, error.message);
        errors++;
        continue;
      }

      // Delete DB record
      await prisma.bookingImage.delete({ where: { id: img.id } });
      deleted++;
    } catch (err) {
      console.error(`[ImageCleanup] Error deleting image id=${img.id}:`, err.message);
      errors++;
    }
  }
  ...
  for (const img of orphans) {
    try {
      await supabase.storage.from(QUOTE_BUCKET).remove([img.storagePath]);
      await prisma.bookingImage.delete({ where: { id: img.id } });
      deleted++;
    } catch (err) {
      console.error(`[ImageCleanup] Orphan delete error id=${img.id}:`, err.message);
      errors++;
    }
  }

  console.log(`[ImageCleanup] Done — deleted: ${deleted}, errors: ${errors}`);
  return { deleted, errors };
}

module.exports = { scheduleImageDeletion, cleanupExpiredImages };
