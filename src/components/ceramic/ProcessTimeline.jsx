      ...
      <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-16" style={{ transition: 'opacity .6s, transform .6s', opacity: visible ? 1 : 0, transform: visible ? 'none' : 'translateY(14px)' }}>
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full mb-4"
            ...
            <span className="text-yellow-400 text-xs font-semibold tracking-widest uppercase">Our Process</span>
          </div>
          <h2 className="text-4xl sm:text-5xl font-black text-white mb-3">
            Six stages of{' '}
            <span style={{ background: 'linear-gradient(135deg,#c9a84c,#f5d376)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>obsession</span>
          </h2>
        ...
        <div className="relative pl-14 sm:pl-16">
          ...
          <div className="absolute left-[26px] sm:left-[30px] top-2 w-px"
            style={{
              background: 'linear-gradient(180deg, #c9a84c, #f5d376)',
            ...
            <div key={title} className="relative mb-10 last:mb-0"
              ...
              <div className="absolute -left-14 sm:-left-16 top-0 w-[52px] sm:w-[60px] flex justify-center">
                <div className="w-11 h-11 rounded-full flex items-center justify-center"
                  style={{
                    background: visible ? 'linear-gradient(135deg,#c9a84c,#f5d376)' : 'rgba(255,255,255,0.06)',
                    border: '2px solid #0d0d0d',
                    boxShadow: visible ? '0 0 18px rgba(201,168,76,0.45)' : 'none',
                    transition: 'background .5s, box-shadow .5s',
                    transitionDelay: `${0.3 + i * 0.13}s`,
                  }}>
                  <Icon className="w-5 h-5" style={{ color: visible ? '#0a0a0a' : '#4b5563', transition: 'color .5s', transitionDelay: `${0.3 + i * 0.13}s` }} />
                </div>
              </div>
              {/* Card */}
              <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
                <div className="flex items-center gap-3 mb-1">
                  <span className="text-yellow-500/70 text-xs font-black tracking-widest">0{i + 1}</span>
